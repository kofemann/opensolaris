...
/*
 * INSTRUCTIONS for updating to a new NFSv4.1 draft:
 *
...
static file_mdsthreshold4 empty_file_mdsthreshold4 = {0, 0, 0, 0, 0};
...
/*
 * encode/decode file-flavored layout hints.  ignore all other types.
 */
bool_t
xdr_file_layouthint4(XDR *xdrs, file_layouthint4 *objp)
{
	layouttype4 lot;
	int xlen;
	...
	if (xdrs->x_op == XDR_ENCODE) {
		lot = LAYOUT4_NFSV4_1_FILES;
		if (!XDR_PUTINT32(xdrs, (int32_t *)&lot))
			return (FALSE);

		xlen = BYTES_PER_XDR_UNIT * 3;
		if (!XDR_PUTINT32(xdrs, (int32_t *)&xlen))
			return (FALSE);

		if (!XDR_PUTINT32(xdrs, (int32_t *)&objp->floh_care))
			return (FALSE);
		if (!XDR_PUTINT32(xdrs, (int32_t *)&objp->floh_util))
			return (FALSE);
		if (!XDR_PUTINT32(xdrs, (int32_t *)&objp->floh_stripe_count))
			return (FALSE);
		return (TRUE);
	}

	/*
	 * Only handle file-flavored layout hints.
	 * Just skip/ignore non-file layout data for decode
	 */
	objp->floh_valid = 0;
	if (!XDR_GETINT32(xdrs, (int32_t *)&lot))
		return (FALSE);
	if (!XDR_GETINT32(xdrs, (int32_t *)&xlen))
		return (FALSE);

	if (lot != LAYOUT4_NFSV4_1_FILES || xlen != BYTES_PER_XDR_UNIT * 3) {
		/*
		 * wrong type or wrong size.  skip it and fail for file
		 * flavored hints (because it isn't encoded incorrectly)
		 */
		if (!XDR_CONTROL(xdrs, XDR_SKIPBYTES, &xlen))
			return (FALSE);
		return (lot != LAYOUT4_NFSV4_1_FILES);
	}

	/*
	 * encode/decode the remaining parts of the file layout hint
	 */
	if (!XDR_GETINT32(xdrs, (int32_t *)&objp->floh_care))
		return (FALSE);
	if (!XDR_GETINT32(xdrs, (int32_t *)&objp->floh_util))
		return (FALSE);
	if (!XDR_GETINT32(xdrs, (int32_t *)&objp->floh_stripe_count))
		return (FALSE);
	objp->floh_valid = 1;
...
void
xdr_get_layouttypes4_inline(uint32_t **iptr, layouttypes4_t *obj)
{
	int i, cnt;
	layouttype4 lot;
	uint32_t *ptr = *iptr;
	...
	*(uint32_t *)obj = 0;
	if ((cnt = IXDR_GET_U_INT32(ptr)) == 0) {
		*iptr = ptr;
		return;
	}

	/*
	 * search for file layout type; ignore other types
	 * There are only 3 layout types.  Instead of failing
	 * if there are more, just skip/ignore them.
	 */
	for (i = 0; i < cnt; i++) {
		lot = IXDR_GET_U_INT32(ptr);
		switch (lot) {
		case LAYOUT4_NFSV4_1_FILES:
			obj->lot_nfsv4_1_files = 1;
			break;
		case LAYOUT4_OSD2_OBJECTS:
			obj->lot_osd2_objects = 1;
			break;
		case LAYOUT4_BLOCK_VOLUME:
			obj->lot_block_volume = 1;
			break;
		default:
			DTRACE_PROBE1(nfsx__e__getlotypes, int, lot);
		}
	}
	*iptr = ptr;
}
...
/*
 * Solaris will only encode and decode file layout types.
 */
bool_t
xdr_layouttypes4(XDR *xdrs, layouttypes4_t *obj)
{
	int i;
	uint32_t count;
	layouttype4  lot;
	...
	/*
	 * ENCODE - server only supports file layouts
	 */
	if (xdrs->x_op == XDR_ENCODE) {
		ASSERT(obj->lot_nfsv4_1_files);
		ASSERT(! obj->lot_osd2_objects && ! obj->lot_block_volume);

		count = obj->lot_nfsv4_1_files;
		if (!XDR_PUTINT32(xdrs, (int32_t *)&count))
			return (FALSE);
		if (count == 0)
			return (TRUE);
	...
	if (!XDR_GETINT32(xdrs, (int32_t *)&count))
		return (FALSE);
	if (count <= 0)
		return (count == 0);
...
/*
 * threshold attributes for file flavored layouts only
 */
void
xdr_get_file_mdsthreshold4_inline(uint32_t **iptr, file_mdsthreshold4 *objp)
{
	uint32_t cnt, blen, bmap, bytecnt;
	uint32_t *ptr = *iptr;
	layouttype4 lot;

	/*
	 * DECODE threshold hints for all layout types but only pay
	 * attention to hits for file-typed layouts.
	 */
	*objp = empty_file_mdsthreshold4;
	if ((cnt = IXDR_GET_U_INT32(ptr)) > TH4_MAX_HINTLIST_LEN) {
		*iptr = ptr;
		return;
	}
	...
	for (; cnt > 0; cnt--) {
		...
		/*
		 * get threshold_item4 bitmap (thi_hintset)
		 * mdsthreshold hint bitmaps should only be 1 word
		 * skip any additional words
		 */
		blen = IXDR_GET_U_INT32(ptr);
		if (blen >= 1) {
			bmap = IXDR_GET_U_INT32(ptr);
			if (blen > 1)
				ptr += (blen - 1);
		}

		/*
		 * get threshold_item4 len and skip the bytes if this isn't
		 * a files-flavored threshold_item4.
		 */
		bytecnt = IXDR_GET_U_INT32(ptr);
		if (lot != LAYOUT4_NFSV4_1_FILES) {
			ptr += bytecnt;
			continue;
		}

		objp->fth_bitmap = bmap;
		if (objp->fth_bitmap & TH4_READ_SIZE_MASK) {
			IXDR_GET_U_HYPER(ptr, objp->fth_rdsize);
			bytecnt -= 8;
		} else
			objp->fth_rdsize = 0;

		if (objp->fth_bitmap & TH4_WRITE_SIZE_MASK) {
			IXDR_GET_U_HYPER(ptr, objp->fth_wrsize);
			bytecnt -= 8;
		} else
			objp->fth_wrsize = 0;

		if (objp->fth_bitmap & TH4_READ_IOSIZE_MASK) {
			IXDR_GET_U_HYPER(ptr, objp->fth_rdiosize);
			bytecnt -= 8;
		} else
			objp->fth_rdiosize = 0;

		if (objp->fth_bitmap & TH4_WRITE_IOSIZE_MASK) {
			IXDR_GET_U_HYPER(ptr, objp->fth_wriosize);
			bytecnt -= 8;
		} else
			objp->fth_wriosize = 0;
		...
		if (bytecnt != 0) {
			/*
			 * bytecnt should be 0.  Skip any remaining bytes.
			 */
			ptr += bytecnt;
		}
	}
 ...
 */
bool_t
xdr_file_mdsthreshold4(XDR *xdrs, file_mdsthreshold4 *objp)
{
	int cnt = 1;
	int blen = 1;
	int bytecnt;
	uint32_t skiplen;
	layouttype4 lot = LAYOUT4_NFSV4_1_FILES;
	uint32_t bmap = (TH4_READ_SIZE_MASK | TH4_WRITE_SIZE_MASK |
	    TH4_READ_IOSIZE_MASK | TH4_WRITE_IOSIZE_MASK);

	if (xdrs->x_op == XDR_FREE)
		return (TRUE);
	/*
	 * MDS only provides threshold hints for file-flavored layouts
	 * and it always uses offset 0 (objp is not used as the source
	 * for encoded threshold values).
	 */
	if (xdrs->x_op == XDR_ENCODE) {
		if (!XDR_PUTINT32(xdrs, &cnt))
			return (FALSE);
		if (!XDR_PUTINT32(xdrs, (int32_t *)&lot))
			return (FALSE);
		if (!XDR_PUTINT32(xdrs, &blen))
			return (FALSE);
		if (!XDR_PUTINT32(xdrs, (int32_t *)&bmap))
			return (FALSE);

		bytecnt = ((BYTES_PER_XDR_UNIT * 2) * 4);
		if (!XDR_PUTINT32(xdrs, &bytecnt))
			return (FALSE);
		if (!xdr_uint64_t(xdrs, &objp->fth_rdsize))
			return (FALSE);
		if (!xdr_uint64_t(xdrs, &objp->fth_wrsize))
			return (FALSE);
		if (!xdr_uint64_t(xdrs, &objp->fth_rdiosize))
			return (FALSE);
		if (!xdr_uint64_t(xdrs, &objp->fth_wriosize))
			return (FALSE);
		return (TRUE);
	}
	...
	/*
	 * DECODE threshold hints for all layout types but only pay
	 * attention to hits for file-typed thresholds.  initialize
	 * files threshold to 0 to simplify error paths.
	 */
	*objp = empty_file_mdsthreshold4;
	if (!XDR_GETINT32(xdrs, &cnt))
		return (FALSE);
	if (cnt  > TH4_MAX_HINTLIST_LEN)
		return (FALSE);
	...
	for (; cnt > 0; cnt--) {
		...
		if (!XDR_GETINT32(xdrs, &blen))
			return (FALSE);
		/*
		 * fail hint bmaps > 1 word
		 */
		if (blen > 0) {
			if (!XDR_GETINT32(xdrs, (int32_t *)&bmap))
				return (FALSE);

			if (blen > 1) {
				skiplen = (blen - 1) * BYTES_PER_XDR_UNIT;
				if (!XDR_CONTROL(xdrs, XDR_SKIPBYTES, &skiplen))
					return (FALSE);
			}
		} else
			bmap = 0;
		...
		/*
		 * decode file-flavored threshold_item4 blob.  skip other
		 * types
		 */
		if (!XDR_GETINT32(xdrs, (int32_t *)&bytecnt))
			return (FALSE);

		if (lot != LAYOUT4_NFSV4_1_FILES) {
			if (!XDR_CONTROL(xdrs, XDR_SKIPBYTES, &bytecnt))
				return (FALSE);
		}

		objp->fth_bitmap = bmap;
		if (bmap & TH4_READ_SIZE_MASK) {
			if (! xdr_uint64_t(xdrs, &objp->fth_rdsize))
				return (FALSE);
			bytecnt -= 8;
		} else
...
xdr_nfs_resop4(XDR *xdrs, nfs_resop4 *objp)
{
	...
	case OP_DESTROY_CLIENTID:
		if (!xdr_DESTROY_CLIENTID4res(xdrs,
		    &objp->nfs_resop4_u.op_destroy_clientid))
			return (FALSE);
		break;
	case OP_RECLAIM_COMPLETE:
...
xdr_layoutrecall_file4(XDR *xdrs, layoutrecall_file4 *objp)
{
	...
	switch (xdrs->x_op) {
	case XDR_ENCODE:
		if (!xdr_encode_nfs41_fh(xdrs, &objp->lor_fh))
			return (FALSE);
		break;
